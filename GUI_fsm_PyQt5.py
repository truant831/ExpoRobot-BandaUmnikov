...
class PlayWorker(QObject):
    ...
    def run(self):
        ...
        while self._running:
            ...
            self.newFrameCamera.emit(frame)
            time.sleep(0.1)  # Задержка (заменить на реальный захват камеры)
            ...
            if was_clicked and card_x>0 and card_y>0:
                ...
                if img_card_detected is not None:
                    ...
                    if frukto_result is None:
                        ...
                        print(f"--- Найдена комбинация {cond} / {group_value} ---")
                    ...
                    if (frukto_result and len(frukto_result) > 0) and n_detected == 8 and not was_clicked:
...
class FruktoStateMachine(QObject):
    ...
    def on_enter_stop(self):
        ...
        # Здесь можно озвучить итоги игры или сохранить логи
